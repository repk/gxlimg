...
# Decompiled from: Python 3.8.9 (default, May 13 2021, 20:22:42) 
# [GCC 10.2.0]
# Embedded file name: acs_tool.py
# Compiled at: 2015-09-17 06:30:19
# Fixed by 2to3 and manual efforts by Geraldo Nascimento <geraldogabriel@gmail.com>
...
class acs_tool(object):
    ...
    def init_acs(self, acs_struct, file_name, bl2):
        seek_position = 0
        file_handler = open(file_name, 'r+b')
        ...
        for key in list(acs_struct.keys()):
            ...
            if isinstance(acs_struct[key], str):
                ...
                acs_struct[key] = file_handler.read(len(acs_struct[key])).decode("ascii")
    ...
    def copy_data(self):
        ...
        for key_addr, key_length in zip(check_excepts, check_excepts_length):
            print(self.acs_des[key_addr], self.acs_des[key_length])
    ...
    def run(self):
        file_des_tmp_handler = open(self.file_des_tmp, 'w+b')
        file_des_handler = open(self.file_des, 'r+b')
        file_des_tmp_handler.write(file_des_handler.read(-1))
...
if __name__ == '__main__':
    ...
    if tool.run():
        exit(1)
# okay decompiling acs_tool.pyc.orig.pyc
